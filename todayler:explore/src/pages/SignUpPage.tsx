...
export function SignUpPage() {
  ...
  const submitSignUp = async (e: React.FormEvent) => {
    ...
    const result = await signUpWithPassword(email, password, buildRedirectUrl('/signup', nextPath));
    if (result.error) {
      setError(result.error);
  ...
  const submitResetRequest = async (e: React.FormEvent) => {
    ...
    const result = await sendPasswordResetEmail(email, buildRedirectUrl('/signup?mode=recover', nextPath));
    if (result.error) {
      setError(result.error);
    } else {
      setMessage(
